...
### NOTE: Implementation modified for 3D convolutions only!!!
...
class ConvNeXtV2Unet(nn.Module):
    ...
    def __init__(self, in_chans=3, out_chans=3,
        ...
        #self.norm = nn.LayerNorm(dims[-1], eps=1e-6) # final norm layer
        self.out_conv = nn.Conv3d(int(dims[0]/2), out_chans, kernel_size=1) # final classifier conv
        self.final_act = nn.Sigmoid() # final activation for generator
        ...
        for i in reversed(range(self.num_stage)):
            upsample_layer = nn.Sequential(
                    ...
                    nn.ConvTranspose3d(dims[i]*2, int(dims[i]/2), kernel_size=2, stride=2),
            )
...
if __name__ == "__main__":
    #model = ConvNeXtV2Unet(in_chans=2, out_chans=2)
    #model = ConvNeXtV2Unet(in_chans=2, out_chans=2, depths=[2, 2, 6, 2], dims=[64, 128, 256, 512])
    #model = ConvNeXtV2Unet(in_chans=2, out_chans=2, depths=[3, 3, 9, 3], dims=[128, 256, 512, 1024])

    model = ConvNeXtV2Unet(in_chans=2, out_chans=2, depths=[2, 2, 8, 2], dims=[128, 256, 512, 1024])
    
    # print(model)
    x = torch.randn(1, 2, 256, 256, 256)
    y = model(x)

    from ptflops import get_model_complexity_info

    with torch.cuda.device(0):
        macs, params = get_model_complexity_info(model, (2, 256, 256, 256), as_strings=True,
