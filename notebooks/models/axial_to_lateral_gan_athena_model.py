...
class AxialToLateralGANAthenaModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.local_rank, 
        ...
        if self.isTrain:  # define discriminators
            self.netD_A_yz = networks.define_D(opt.output_nc, opt.ndf, opt.netD,
                                               opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.local_rank,
                                               False, dimension=self.dis_dimension)
            ...
            self.netD_B_xz = networks.define_D(opt.input_nc, opt.ndf, opt.netD, 
                                               opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.local_rank,
                                               False, dimension=self.dis_dimension)
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.AdamW(itertools.chain(self.netG_A.parameters(), self.netG_B.parameters()),
                                                lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizer_D = torch.optim.AdamW(itertools.chain(self.netD_A_yz.parameters(), self.netD_A_xy.parameters(), self.netD_A_xz.parameters(),
    ...
    def set_input(self, input):
        ...
        self.l1_mask = input['l1_mask'].to(self.device)
    ...
    def forward(self):
        ...
        """
        self.fake_pre = self.netG_A(self.real)  # G_A(A)

        self.fake = self.fake_pre * (1-self.l1_mask) + (self.real * self.l1_mask)
    ...
    def backward_D_basic(self, netD, real, fake, slice_axis_real, slice_axis_fake):
        ...
        loss_D.backward()
        torch.distributed.all_reduce(loss_D, op=torch.distributed.ReduceOp.AVG)
        return loss_D
    ...
    def backward_G(self):
        ...
        # Add L1 loss to XY layers
        #self.loss_l1_xy = torch.nn.functional.l1_loss(self.fake[:, 0, :, :, :] * self.l1_mask, self.real[:, 0, :, :, :] * self.l1_mask) * 10

        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A# + self.loss_l1_xy
