...
from .convnext_v2 import ConvNeXtV2, ConvNeXtV2Unet
...
def init_weights(net, init_type='normal', init_gain=0.02, local_rank=None):
...
def init_net(net, init_type='normal', init_gain=0.02, local_rank=None):
    """Initialize a network: 1. register GPU device (with multi-GPU support); 2. initialize the network weights
    Parameters:
    ...
    net.to(local_rank)
    net = torch.nn.parallel.DistributedDataParallel(net, device_ids=[local_rank])#, process_group=pg)
    return net


def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02,
             local_rank=None, kernel_size=9, given_psf=None, noise_setting = None, dimension = 3):
    """Create a generator
    ...
    if netG == 'unet_convnext_xlarge':
        net = ConvNeXtV2Unet(input_nc, output_nc, depths=[3, 3, 9, 3], dims=[96, 192, 384, 768])
    elif netG == 'unet_convnext_large':
        net = ConvNeXtV2Unet(input_nc, output_nc, depths=[2, 2, 6, 2], dims=[64, 128, 256, 512])
    elif netG == 'unet_convnext_small':
        net = ConvNeXtV2Unet(input_nc, output_nc, depths=[2, 2, 6, 2], dims=[32, 64, 128, 256])
    elif netG == 'unet_twoouts':
        ...
        net = Unet_vanilla(intput_nc, output_nc, norm_layer=norm_layer, dimension=dimension)
    ...
    return init_net(net, init_type, init_gain, local_rank)

def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, 
             local_rank=None, use_sigmoid=False, dimension =3):
