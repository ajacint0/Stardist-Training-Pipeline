...
def update_lr(optimizer, scheduler, opt, metric=None, do_print=True):
    """
    ...
    if do_print: print('learning rate %.7f -> %.7f' % (old_lr, lr))
...
def get_scheduler(optimizer, opt, init_lr=None):
    ...
    if opt.lr_policy == "none":
        ...
        scheduler = lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=opt.T_0, T_mult=opt.T_mult, eta_min=opt.eta_min, last_epoch=opt.last_epoch)
    else:
