...
class StarDistBase(nn.Module):
    def __init__(self, opt, rays=None):
        ...
        self.local_rank = int(os.environ["LOCAL_RANK"])# ALEX commented out
        self.global_rank = int(os.environ["RANK"])# ALEX commented out
        ...
        if hasattr(opt, "load_epoch") and opt.load_epoch not in (None, ""):
            #self.net = define_stardist_net(opt) # Alex added this
    ...
    def optimize_parameters(self, batch, epoch=None):
        ...
        #if prob_class is not None:
        #    assert opt.n_classes is not None, f"'prob_class' (type={type(prob_class)}) not  None in batch but opt.n_classes is None"
        #    prob_class = prob_class.to(self.device)
        #else:
    ...
    def predict_instance(
        ...
        self.net.eval() #Alex commented out
    ...
    def load_state(self, name=None, load_path=None):
        ...
        if self.net is None:
            ...
            #print( self.net.load_state_dict( state['model_state_dict'] ) ) #Alex commented out
        else:
            if isinstance(self.net, torch.nn.parallel.DistributedDataParallel):
                print()
                #print( self.net.module.load_state_dict( state['model_state_dict'] ) ) #Alex commented out
            else:
                #print( self.net.load_state_dict( state['model_state_dict'] ) ) #Alex commented out
                print()
