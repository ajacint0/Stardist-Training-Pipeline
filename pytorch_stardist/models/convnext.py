...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6):
        super().__init__()
        self.Conv = nn.Conv3d
        self.dwconv = self.Conv(dim, dim, kernel_size=7, padding=3, groups=dim) # depthwise conv
...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=21841, 
                 ...
                 layer_scale_init_value=1e-6, head_init_scale=1., out_indices=[0, 1, 2, 3], grid=None
                 ):
        super().__init__()
        ...
        if grid is None:
            grid = 4
        stem = nn.Sequential(
            self.Conv(in_chans, dims[0], kernel_size=grid, stride=grid),
        ...
        for i in range(3):
            downsample_layer = nn.Sequential(
                    ...
                    self.Conv(dims[i], dims[i+1], kernel_size=2, stride=2),
            )
    ...
    def _init_weights(self, m):
        if isinstance(m, (self.Conv, nn.Linear)):
